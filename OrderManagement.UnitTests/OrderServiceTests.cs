...
namespace OrderManagement.UnitTests;
    ...
    public class OrderServiceTests
    {
        ...
        public void CreateOrder_ValidOrder_StoresOrderAndUpdatesStock()
        {
            ...
            int initialStock1 = _mockProducts[0].Stock;
            int initialStock2 = _mockProducts[1].Stock;
        
            List<OrderItem> orderItems = new List<OrderItem>
            {
                new OrderItem { ProductId = _mockProducts[0].Id, Quantity = 2 },
                new OrderItem { ProductId = _mockProducts[1].Id, Quantity = 3 }
            };
            DateTime now = DateTime.Now;
        
            _mockUserRepo.Setup(repo => repo.Get(userId)).Returns(_mockUsers[0]);
            _mockProductRepo.Setup(repo => repo.GetAll()).Returns(new List<Product> { _mockProducts[0], _mockProducts[1] });
            ...
            Assert.True(order.OrderItems.Any(oi => oi.ProductId == _mockProducts[0].Id && oi.Quantity == 2));
            Assert.True(order.OrderItems.Any(oi => oi.ProductId == _mockProducts[1].Id && oi.Quantity == 3));
            ...
            _mockOrderRepo.Verify(repo => repo.Add(It.Is<Order>(o =>
                ...
                o.OrderItems.Any(oi => oi.ProductId == _mockProducts[0].Id && oi.Quantity == 2) &&
                o.OrderItems.Any(oi => oi.ProductId == _mockProducts[1].Id && oi.Quantity == 3)
            )), Times.Once());
            
            Assert.Equal(initialStock1 - 2, _mockProducts[0].Stock);
            Assert.Equal(initialStock2 - 3, _mockProducts[1].Stock);

            if (_mockProducts[0].Stock != 0)
                Assert.NotEqual(initialStock1, _mockProducts[0].Stock);
            
            if(_mockProducts[1].Stock != 0)
                Assert.NotEqual(initialStock1, _mockProducts[1].Stock);
        }
        ...
        [Fact]
        public void AdvanceOrderStatus_InvalidOrderId_ThrowsException()
        {
            // Arrange
            Guid invalidOrderId = Guid.NewGuid();
            _mockOrderRepo.Setup(repo => repo.Get(invalidOrderId)).Returns((Order)null);
